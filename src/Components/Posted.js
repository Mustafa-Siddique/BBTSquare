...
export default function Posted({ add }) {
  ...
  const renderProjects = (projects, index) => {
    return (
      <div
        className={
          projects.wallet === add ? "col-md-6 my-2" : "col-md-6 my-2 d-none"
        }
        key={index}
      >
        <div
          className="card"
          style={{ minWidth: "18rem", borderRadius: "5px" }}
        >
          <div className="card-body">
            ...
            <h5 style={{ position: "absolute", right: "25px", top: "15px" }}>
              {projects.cost} BNB
            </h5>
            <h6 className="card-subtitle mb-2 text-light">
              Posted by: {projects.wallet}
            </h6>
